...
const swiftTiles = (direction: number) => {
  ...
  let baseVector = f.mobileNow ? 7 : 15; // 모바일 여부에 따라 기본 이동 속도 조절
   ...
   * @param {number} i - select.sevenTiles 배열 내 타일의 인덱스.
   * @param {number} oDegree - 타일의 시작 각도.
   * @param {number} nDegree - 타일의 목표 각도.
   */
  const tileM = (i: number, oDegree: number, nDegree: number) => {
    const s = t[i]; // 대상 스프라이트
    let thisDegree = oDegree; // 현재 각도
    const vector = (nDegree - oDegree) / baseVector; // 각도 변화량 (이동 속도)
    let index = 0; // 애니메이션 프레임 카운터
  ...
  const tileA = (directions: number) => {
    let d: number; // 타일의 각도
    let t: PIXI.Texture; // 타일의 텍스처
    let s: PIXI.Sprite; // 새로 생성될 스프라이트

    switch (directions) {
      case LEFT: // 왼쪽으로 이동할 때 (새 타일이 오른쪽에 추가)
        select.startNum = plus(select.startNum); // 시작 인덱스 업데이트
        select.endNum = plus(select.endNum); // 끝 인덱스 업데이트
        d = select.degrees[6]; // 가장 오른쪽 각도
        t = PIXI.Assets.get(select.sFileNames[select.endNum]); // 새 타일의 텍스처 로드
        s = makeSelectSprite(d, t, select.selectC2, select.fSize); // 새 스프라이트 생성
        select.sevenTiles.push(s); // 배열에 추가
        s.alpha = 0; // 초기 투명도 0
        break;
      case RIGHT: // 오른쪽으로 이동할 때 (새 타일이 왼쪽에 추가)
        select.startNum = minus(select.startNum); // 시작 인덱스 업데이트
        select.endNum = minus(select.endNum); // 끝 인덱스 업데이트
        d = select.degrees[0]; // 가장 왼쪽 각도
        t = PIXI.Assets.get(select.sFileNames[select.startNum]); // 새 타일의 텍스처 로드
        s = makeSelectSprite(d, t, select.selectC2, select.fSize); // 새 스프라이트 생성
        select.sevenTiles.unshift(s); // 배열 맨 앞에 추가
        s.alpha = 0; // 초기 투명도 0
        break;
    }
  ...
  switch (direction) {
    case LEFT:
      ...
      tileV(0); // 가장 왼쪽 타일 사라지게 함
      t.shift(); // 배열에서 제거
      tileA(LEFT); // 새 타일 나타나게 함
      break;
    case RIGHT:
      ...
      tileV(t.length - 1); // 가장 오른쪽 타일 사라지게 함
      t.pop(); // 배열에서 제거
      tileA(RIGHT); // 새 타일 나타나게 함
      break;
  }
};
...
const getPositionAndSize = (
  ...
  let radius = (floorSize * 4) / 5; // 회전 반지름
  let tSize = floorSize / 2; // 기본 타일 크기
  // 원형 경로를 따라 x, y 위치 계산
  ...
  let angle = degree + 90; // 실제 표시될 각도 조정
